 ...
 CfromFASTAv4<-function(fasta, chr, out.dir, write.output)
 {
   ...
   CleanEnvir <- function(pattern)
   {
     rm(list = ls(envir=globalenv())[
      ...
      cat("- Converting DNA .....", "\n")
      fasta.minus<-complement(fasta)
      fasta.plus<-DNAString(paste(fasta, collapse=""))
      fasta.minus<-DNAString(paste(fasta.minus, collapse=""))
              ...
              for (c in 1:length(strand.temp))
              {
                ...
                for (p in 1:length(context.temp))
                {
                  counter<-counter+1
                   ...
                   {
                     match.out<-matchPattern(context.temp[p], subject=fasta.plus)
                   ...
                   if (names(cytosine.patterns)[[s]] == "-")
                   {
                     match.out<-matchPattern(context.temp[p], subject=fasta.minus)
